...
type RetentionSettings = { seconds: number };
...
export const getRetentionSeconds = async () => {
...
export const normalizeEmailPayload = (item: unknown) => {
...
export const cleanupExpiredMessages = async (address: string) => {
...
/**
 * Shared inbox logic used by BOTH the internal web UI route (/api/inbox)
 * and the public developer API (/api/v1/inbox).
 */
export const getInboxEmails = async (address: string, forceResync = false) => {
  ...
  return { emails: normalizedEmails, imapDebug: imapResult.debug };
};
